...
if(!file.exists(here("data", "raw", "0043_mostajabi_ts_raw.csv"))){
  osf_retrieve_file("https://osf.io/zqd5w") |>
    osf_download(path = here("data", "raw"))

  # rename data
  file_name <- osf_retrieve_file("https://osf.io/zqd5w") |> pull(name)
  file.rename(here("data", "raw", file_name), here("data", "raw", "0043_mostajabi_ts_raw.csv"))
}

df_raw <- read.csv(here("data", "raw", "0043_mostajabi_ts_raw.csv"))
...
df <- df |>
  select(!c(total_ema, first_ema, pa, neg_aff, pa_interaction, neg_aff_interaction, last_ema,
            total_daily_ema, order_daily_ema, first_ema_day))
...
if(check_results == "Data are clean."){
  write_tsv(df, here("data", "clean", "0043_mostajabi_ts.tsv"))
}

# Create metadata ---------------------------------------------------------
write_metadata("0043", meta_data = meta_data, variable_data = variable_data)