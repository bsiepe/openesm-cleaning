...
# Data --------------------------------------------------------------------
# load data
if(!file.exists(here::here("data", "raw", "0050_pronizius_ts_raw.xlsx"))){
  osf_retrieve_file("https://osf.io/qvtgc") |>
    osf_download(path = here::here("data", "raw"))

  # rename data
  file_name <- osf_retrieve_file("https://osf.io/qvtgc") |> pull(name)
  file.rename(here::here("data", "raw", file_name), here::here("data", "raw", "0050_pronizius_ts_raw.xlsx"))
}

# read data
df_raw <- readxl::read_xlsx(here("data", "raw", "0050_pronizius_ts_raw.xlsx"))
...
df_demographics <- df |>
  select(id, age, gender, pss, loneliness, depression, virusconcern, helpers) |>
  group_by(id) |>
  distinct()
...
df <- df |>
  select(!c(age, gender, pss, loneliness, depression, virusconcern, helpers))
...
if(check_results == "Data are clean."){
  write_tsv(df, here("data", "clean", "0050_pronizius_ts.tsv"))
}

# Create metadata ---------------------------------------------------------
write_metadata("0050", meta_data = meta_data, variable_data = variable_data)